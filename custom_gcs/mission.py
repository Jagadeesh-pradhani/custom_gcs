...
class AutonomousMission():
    ...
    def __init__(self, parent):
        ...
        print("AutonomousMission object created")
    ...
    def ArmAndTakeoff(self, aTargetAltitude):
        ...
        while not self.node._vehicle.is_armable:
            print(" Waiting for vehicle to initialise...")
            time.sleep(1)

        print("Arming motors")
        # Copter should arm in GUIDED mode
        self.node._vehicle.mode = VehicleMode("GUIDED")
        self.node._vehicle.armed = True

        while not self.node._vehicle.armed:
            print(" Waiting for arming...")
            time.sleep(1)

        print("Taking off!")
        self.node._vehicle.simple_takeoff(aTargetAltitude)  # Take off to target altitude

        # Wait until the vehicle reaches a safe height before processing the goto (otherwise the command
        #  after Vehicle.simple_takeoff will execute immediately).
        while True:
            # print(" Altitude: ", self.node._vehicle.location.global_relative_frame.alt)
            if self.node._vehicle.location.global_relative_frame.alt >= aTargetAltitude * 0.95:  # Trigger just below target alt.
    ...
    def GotoLocation(self, to_lat, to_lon, alt):
        ...
        self.node._vehicle.simple_goto(target_Location, groundspeed=1.5)

        while self.node._vehicle.mode.name == "GUIDED":
            # print "DEBUG: mode: %s" % vehicle.mode.name
            remainingDistance = self.get_distance_metres(self.node._vehicle.location.global_frame, target_Location)
    ...
    def TriggerServo(self, num, PWM):
        msg = self.node._vehicle.message_factory.command_long_encode(
        ...
        self.node._vehicle.send_mavlink(msg)
        print("payload dropped...")

    def send_ned_velocity(self, velocity_x, velocity_y, velocity_z):
        msg = self.node._vehicle.message_factory.set_position_target_local_ned_encode(
    ...
    def send_ned_velocity1(self, velocity_z, to_alt):
        ...
        while True:
            self.node._vehicle.send_mavlink(msg)
            # print(" Altitude: ", self.node._vehicle.location.global_relative_frame.alt)
            if self.node._vehicle.location.global_relative_frame.alt >= to_alt - 1 and self.node._vehicle.location.global_relative_frame.alt <= to_alt + 1:
                print("Reached target altitude")
                break
            time.sleep(1)
    ...
    def landt(self):
        ...
        while(1):
            ...
            if x != h_x and y != h_y:
                ...
                break
            # print(" Altitude: ", self.node._vehicle.location.global_relative_frame.alt)
            if self.node._vehicle.location.global_relative_frame.alt <=2.5:
                print(f"Reached target : {self.node._vehicle.location.global_frame.lat}, {self.node._vehicle.location.global_frame.lon}")
                return


    def mission(self):
        # self.node._vehicle = self.ConnectToVehicle()
        alt = 10
        print(self.node._vehicle.location.global_frame.lat, self.node._vehicle.location.global_frame.lon)
        ...
        self.node._vehicle.mode = VehicleMode("RTL")
        # self.node._vehicle.close()
